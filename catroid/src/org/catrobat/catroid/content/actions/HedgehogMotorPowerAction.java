...
import static at.pria.hedgehog.protocol.proto.MotorP.POWER;

public class HedgehogMotorPowerAction extends TemporalAction {

	private static final String TAG = HedgehogMotorPowerAction.class.getSimpleName();

	private Formula portFormula;
	private Formula powerFormula;

	private Sprite sprite;

	private int portInterpretation;
	private int powerInterpretation;
	...
	protected void begin() {
		...
		try {
			powerInterpretation = powerFormula == null ? Integer.valueOf(0) : powerFormula
					.interpretInteger(sprite);
		} catch (InterpretationException interpretationException) {
			powerInterpretation = 0;
			Log.d(TAG, "Formula interpretation for this specific Brick failed. (power)",
					interpretationException);
		}
	}
	...
	public void setPowerFormula(Formula powerFormula) {
